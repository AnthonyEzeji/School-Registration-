...
await axios.patch(`3.87.187.44:5001/api/users/${currentFaculty.UserID}`, {Password,Email,UserType}).then(res=>{
    console.log(res.data)
})
    }
    ...
    async function handleDeleteClick(e, cellValues){
        ...
        await axios.delete('3.87.187.44:5001/api/advisor', {data:{FirstName:cellValues.row.FirstName, LastName: cellValues.row.LastName}})
    }
    useEffect(async () => {
       await axios.get(`3.87.187.44:5001/api/advisor/advisees/${JSON.parse(window.sessionStorage.getItem('current-faculty')).UserID}`).then(res=>{
...
await axios.post(`3.87.187.44:5001/api/advisor/${JSON.parse(window.sessionStorage.getItem('current-faculty')).UserID}`, {student, Faculty_ID:JSON.parse(window.sessionStorage.getItem('current-faculty')).UserID})
          }
    return (
