...
export default function DrawerMenu() {
  ...
  const list = (anchor) => (
    <Box
      sx={{ width: anchor === 'Faculty Panel' || anchor === 'bottom' ? 'auto' : 250 }}
      role="presentation"
      onClick={toggleDrawer(anchor, false)}
      onKeyDown={toggleDrawer(anchor, false)}
    >
       <Divider> <h5>Faculty Panel</h5></Divider>
      <List>
        {[ 'Schedule', 'View My Classes'].map((text, index) => (
      ...
      <List>
        {['Search Student','View Degree Audit'].map((text, index) => (
          <ListItem button key={text} onClick={handleClick}>
            <ListItemIcon>
              {index % 2 === 0 ? <InboxIcon /> : <InboxIcon />}
            </ListItemIcon>
