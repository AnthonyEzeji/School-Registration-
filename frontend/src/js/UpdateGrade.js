...
var navigate = useNavigate()
  ...
  useEffect(async() => {
    await axios.get(`3.87.187.44:5001/grades/${JSON.parse(window.sessionStorage.getItem('curr-class'))}/${JSON.parse(window.sessionStorage.getItem('current-student')).UserID}`).then(res=>{
      setGrade(res.data)
...
await axios.post('3.87.187.44:5001/api/grades',{Student_ID:JSON.parse(window.sessionStorage.getItem('current-student')).UserID, CRN:JSON.parse(window.sessionStorage.getItem('curr-class')), Grade:grade} ).then(res=>{
  console.log(res.data)
})
